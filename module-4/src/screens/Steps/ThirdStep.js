...
const ThirdStep = ({navigation}) => {
  const thirdStep = steps.filter(step => {
    return step.id === '3';
  });

  const {id, name, text, instructions} = thirdStep[0];

  const backStep = () => navigation.navigate(ONBOARDING_SECOND_STEP);
  const forwardStep = () => navigation.navigate(ONBOARDING_FOUR_STEP);
