...
import {ONBOARDING_SECOND_STEP, ONBOARDING_HOME} from '../../router/routes';
...
const FirstStep = ({navigation}) => {
  ...
  const {id, name, text, instructions} = firstStep[0];

  const backStep = () => navigation.navigate(ONBOARDING_HOME);
  const forwardStep = () => navigation.navigate(ONBOARDING_SECOND_STEP);
